-- Durable Action Offers, Intents, audits and Events for the typed runtime.
...
CREATE INDEX typed_intent_pending_due
    ON kernel.typed_intent (accepted_at, id) WHERE status = 'PENDING';
CREATE INDEX typed_intent_retry_due
    ON kernel.typed_intent (next_attempt_at, id) WHERE status = 'RETRY_WAIT';
CREATE INDEX typed_intent_claimed_expiry
    ON kernel.typed_intent (lease_until, id) WHERE status = 'CLAIMED';
CREATE INDEX typed_intent_tenant_history
    ON kernel.typed_intent (tenant_id, accepted_at, id);
...
BEGIN
    ...
    VALUES (gen_random_uuid(), selected_id, selected_tenant, audit_sequence, selected_previous_status, 'CLAIMED', claimed_at,
        CASE WHEN selected_previous_status = 'CLAIMED'
            THEN 'expired worker lease reclaimed' ELSE 'worker lease claimed' END, claim_token);
